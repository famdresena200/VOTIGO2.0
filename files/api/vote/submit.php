...
try {
    ...
    $hasUserColumn = votes_has_user_column($pdo);
    $alreadyVotedQuery = $hasUserColumn
        ? 'SELECT id_vote FROM votes WHERE id_election = ? AND (token_anonyme = ? OR id_user = ?) LIMIT 1'
        : 'SELECT id_vote FROM votes WHERE token_anonyme = ? AND id_election = ? LIMIT 1';
    $stmt = $pdo->prepare($alreadyVotedQuery);
    $hasUserColumn
        ? $stmt->execute([$id_election, $token, $id_user])
    ...
    try {
        if ($hasUserColumn) {
            $stmt = $pdo->prepare(
                'INSERT INTO votes (id_candidat, id_election, id_user, token_anonyme, date_vote)
                 VALUES (:id_candidat, :id_election, :id_user, :token_anonyme, NOW())'
            );
            $stmt->execute([
    ...
    if ((int)($e->errorInfo[1] ?? 0) === 1062) {
        api_error('Already voted in this election', 409, ['vote' => 'Vous avez déjà voté pour cette élection.']);
    }
